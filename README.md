...
Edita `app/model/db.php` y ajusta las credenciales locales:
...
> Asegúrate de que `$env = 'local';` está activo en esa misma función.

### 6. Configurar Google OAuth

1. Ve a [Google Cloud Console](https://console.cloud.google.com) → APIs y servicios → Credenciales.
2. Crea unas credenciales **OAuth 2.0 para aplicación web**.
3. Añade como URI de redirección autorizado:
   ```
   http://localhost/mi-spotify/public/google-callback.php
   ```
4. Edita `app/services/GoogleClient.php` con tu **Client ID** y **Client Secret**.

### 7. Configurar Stripe

Las claves de Stripe se pueden definir como variables de entorno (recomendado) o directamente en `public/checkout.php`:
...
Para pruebas usa la tarjeta de test de Stripe: **4242 4242 4242 4242**, cualquier fecha futura y cualquier CVC.

### 8. Ajustar límites de subida de PHP

El archivo `public/user.ini` ya contiene los valores necesarios para XAMPP. Si tu configuración no los respeta, edita también tu `php.ini`:
...
Reinicia Apache tras cualquier cambio.

### 9. Permisos de carpetas de uploads

En Windows con XAMPP no suele ser necesario hacer nada. En Linux / Mac:
...
```
http://localhost/mi-spotify/public/
```
...
Ve a `http://localhost/mi-spotify/public/altaAdmin.php` e introduce la **clave de administrador**.

Por defecto es `180705`, o el valor de la variable de entorno `ADMIN_REGISTER_KEY`.
...
mi-spotify/
├── app/
│   ├── controller/       # Lógica de negocio (Auth, Song, User)
│   ├── model/            # Acceso a BD (db.php, songModel, userModel)
│   ├── services/         # Servicios externos (GoogleClient)
│   └── views/            # Vistas parciales (profile, account)
├── public/               # Punto de entrada — páginas accesibles
│   ├── assets/
│   │   ├── css/          # Hojas de estilo
│   │   └── js/           # Scripts del reproductor y UI
│   └── *.php             # Páginas (dashboard, player, checkout…)
├── uploads/
│   ├── music/            # Archivos de audio subidos
│   └── artCover/         # Portadas de álbum
...
| ------------------------ | ------------------------------ | ------------------------- |
| `ADMIN_REGISTER_KEY`     | Clave para crear cuentas admin | `180705`                  |
| `STRIPE_SECRET_KEY`      | Clave secreta de Stripe        | hardcoded en checkout.php |
| `STRIPE_PUBLISHABLE_KEY` | Clave pública de Stripe        | hardcoded en checkout.php |

> ⚠️ Para producción, define estas variables en el servidor y **nunca** las subas a control de versiones.
